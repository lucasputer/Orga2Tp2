...
struct dictionary_t* dictionary_create_size(uint32_t size){
...
struct dictionary_t* dictionary_create(){
	return dictionary_create_size(INITIAL_DICT_ENTRIES_SIZE);
}
...
uint32_t dictionary_add_entry(struct dictionary_t* dict, char* key){
	...
	//actualizar el size cambiando solo entries
	if(dict->size + 1 >= dict->max_size){
...
void obdd_mgr_print(obdd_mgr* mgr){
	printf("[OBDD MANAGER]\nMgr: %d\n", mgr->ID);
	printf("Mgr.Dict:\n");
	// printf("greatest_var_ID:%u\n",mgr->greatest_var_ID);
	// printf("greatest_node_ID:%u\n",mgr->greatest_node_ID);
	// printf("size:%u\n", mgr->vars_dict->size);
	uint32_t i;
...
obdd* obdd_exists(obdd* root, char* var){ 
	obdd* obdd_true = obdd_restrict(root, var, true);
	obdd* obdd_false = obdd_restrict(root, var, false);
	return obdd_apply_or(obdd_true,obdd_false);;
}
...
obdd* obdd_forall(obdd* root, char* var){ 
	obdd* obdd_true = obdd_restrict(root, var, true);
	obdd* obdd_false = obdd_restrict(root, var, false);
	return obdd_apply_and(obdd_true,obdd_false);;
}
...
void obdd_node_print_aux(obdd_mgr* mgr, obdd_node* root){
	if(is_constant(mgr,root)){
		printf("->");
		if(is_true(mgr,root)){
			printf("1\n");
...
void obdd_node_print(obdd_mgr* mgr, obdd_node* root, uint32_t spaces){
	if(spaces == 0)
		printf("\n");
	for(int i = 0; i < (int) spaces; i++)
		printf(" ");
	char* var_name = dictionary_key_for_value(mgr->vars_dict,root->var_ID);
	printf("%s", var_name);
	obdd_node_print_aux(mgr,root->high_obdd);
	if(!is_constant(mgr,root->high_obdd))
	...
	for(int i = 0; i < (int)spaces; i++)
		printf(" ");
	printf("(!%s)", var_name);
	obdd_node_print_aux(mgr,root->low_obdd);
	if(!is_constant(mgr,root->low_obdd))
		obdd_node_print(mgr,root->low_obdd,spaces+1);
}


// Devuelve true si el nodo pasado como par´ametro es igual al nodo constante true, la comparaci´on debe
// hacerse utilizando mgr->true obdd->root obdd->var ID == root->var ID.
